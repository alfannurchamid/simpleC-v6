...
$sqlite2 = new SQLiteTabulation($pdo2);
$selector2 = new SQLiteQuerySelect($pdo2);
...
switch ($s) {
    ...
    case "progSpptKosong":
        ...
        if (file_exists("tmp/dataSppt.xls")) {
            ...
            foreach ($reader as $row) {
                ...
                $pajaki  = $row[7];
                if ($noi != "") {
                    $sqlite2->insertSppt($noi, $tahuni, $nopi, $no_induki, $namabarui, $alamat_wajibi, $alamat_objeki, $pajaki);     //di masukan ke data induk dan di tandai di tb_baru            
                }
                usleep(250000);
                if (ob_get_level() > 0) {
                    ob_end_flush();
    ...
    case "hapus_sppt":
        ...
        include "hapus_sppt.php";
        $count = $selector2->getSpptCountTotal(); //mengambil jumlah sppt dari db
...
<?php
            ...
            $datNopPraha = $selector2->getPraha($dtnop);
            ...
            if ($g2 !== $dtnop) {
                ...
                if (!isset($dtnop)) {
                    ...
                    //input hapus
                    $sqlite2->inputHapus($dtno, $dtahun, $dtnop, $dtno_induk, $namabaru, $dtalamat_wp, $dtalamat_op, $dtpajak_t, $th_sek);
                }
        ...
        $sqlite2->HapusTabPraha();
        $sqlite2->BuatTabPraha();
    ...
    case "progExcelsppt":
        ...
        if (file_exists("tmp/dataSppt.xls")) {
            ...
            foreach ($reader as $row) {
                ...
                if ($no_ubah_ini !== $nop) {
                    ...
                    $sqlite2->insertSppt($no, $tahun, $nop, $no_induk, $namabaru, $alamat_wajib, $alamat_objek, $pajak);     //di masukan ke data induk dan di tandai di tb_baru
                    //	echo "sppt ".$nop." berhasil dimasukan ke induk";	
                    //di masukan ke data induk dan di tandai di tb_baru
                    $sqlite2->insertSpptbaru($tahun, $nop, $namabaru);
                } else {
                    //	echo "<br>";
                    //	echo ".  no.".$nop.". ada  nama  ";
                    //jika ada maka di cek
                    $perubahan = "";
                    $rowceknama = $selector2->getNop($nop);
                    ...
                    if ($namabaru !== $namak) {
                        ...
                        $sqlite2->updateSpptNama($namabaru, $nop);
                    }
                    ...
                    if ($var_pajak == $pajak) {
                        ...
                        $sqlite2->updateSpptPajak($pajak, $nop);
                    }
                    ...
                    if ($perubahan !== "") {
                        $namak1 = $perubahan;
                        $sqlite2->insertperu($nop, $namak1, $tahun);
                        //	echo "<br>";
                        //	echo "----".$perubahan."---------------------------------------------------------------------------<br>";       
                    }
                    //input perubahan
        ...
        break;
    case "getCountsppt":
        $dataAset = $selector2->getSpptCountTotal();
        $proc = $dataAset;
        echo $proc;
        break;
    case "getCountPraha":
        $dataAset = $selector2->getPrahaCountTotal();
        $proc = $dataAset;
        echo $proc;
        break;
    case "dbs":
