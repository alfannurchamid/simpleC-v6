...
switch ($s) {
    ...
    case "new":
        $wNoC = $_POST["wNoC"];
        $wNama = $_POST["wNama"];
        $wAlamat = $_POST["wAlamat"];
        $aPersil = $_POST["aPersil"];
        $aKategori = $_POST["aKategori"];
        $aKelas = $_POST["aKelas"];
        $aLuas = $_POST["aLuas"];

        $warga = $selector->getWargaById($wNoC);
        $response = "";
        if (count($warga) == 0) {
            $sqlite->insertWarga($wNoC, $wNama, $wAlamat);
            for ($c = 0; $c < count($aPersil); $c++) {
                if ($aKategori[$c] == 'BASAH') {
                    $kelasan[$c] = 'S.' . $aKelas[$c];
                } else {
                    $kelasan[$c] = 'D.' . $aKelas[$c];
                }
    ...
    case "cnv":
        ...
        if ($luasOld <= $aLuas) {
            ...
            $periwayatan = $sqlite->insertRiwayatKet($asalId, $aPersil, $kelasan, $aLuas, $sb, $cAsal, $nc, $tgl, $ket);
